class Search
    ...
    def ventas
		if @keywords.present?
		    ventas = Venta.where(venta_condition).order(number: :desc).offset(@offset).limit(@page_size)
		    @number_of_records = Insumo.where(description_condition).count
	    else
		    ventas = Venta.where(state: "confirmed").order(number: :desc).offset(@offset).limit(@page_size)
			@number_of_records = Venta.where(state: "confirmed").count
	    end

		return ventas, number_of_pages
	end
    
	def compras
		if @keywords.present?
		    compras = Compra.where(compra_condition).order(number: :desc).offset(@offset).limit(@page_size)
		    @number_of_records = Insumo.where(descripcion_condition).count
	    else
		    compras = Compra.where(state: "confirmed").order(number: :desc).offset(@offset).limit(@page_size)
			@number_of_records = Compra.where(state: "confirmed").count
	    end

		return compras, number_of_pages
	end
