class SaleDetailsController < ApplicationController
	...
	def new
		...
		@sale_details.insumo = Insumo.first
	end

	def create
		insumo_exists = false
		insumo_id = params[:sale_details][:insumo_id].to_i
  ...
  private
    ...
    def sale_details_params
      params.require(:sale_details).permit(:id, :sale_id, :insumo_id, :insumo_description, :number, :qty, :price, :_destroy)
    end
end